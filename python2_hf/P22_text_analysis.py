...
# számláló függfény definiálása
def word_counter(list_in):
    counter = dict()
    for word in list_in:
        if word in counter:
            counter[word] += 1
...
# string formázása kisbetűssé
text = text.lower()

# string listává alakítása
words_list = text.split()
words_list.sort()
print(words_list)

# függvény hívása listára és kiíratása
print(word_counter(words_list))

# eredmény kiírása lista szerűen
for key, value in word_counter(words_list).items():
    print("szó: " + str(key) + ", előfordulások száma: " + str(value))